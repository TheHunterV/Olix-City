...
local function checkDrugRecipes(ingrQuant, plyId, labId)
    ...
    for drugName, drugData in pairs(config.drugs) do
        ...
        if(canDoThisDrug and (playerUsedIngredientCount == totalIngredientsNumber) and drugData.allowedLabs[labId]) then
            if(canPlayerCarry(plyId, drugName, drugData.baseRecipeReward)) then
                ...
                TriggerClientEvent('lds_drugs_system:correctIngredients', plyId, drugName, drugLabel)
    ...
    if(next(ingrQuant) ~= nil) then
        TriggerClientEvent('lds_drugs_system:wrongIngredients', plyId)
    end
...
ESX.RegisterServerCallback('lds_drugs_system:hasAnyDrug', function(playerId, cb)
...
RegisterNetEvent('lds_drugs_system:alarmPolice')
AddEventHandler('lds_drugs_system:alarmPolice', function(isPlane, veh, justStarted, coords) 
    ...
    for i=1, #xPlayers, 1 do
        ...
        if(xPlayer.job.name == "police") then
            ...
            TriggerClientEvent('lds_drugs_system:addAlertBlip', xPlayer.source, veh, coords)
        end
    end
end)

ESX.RegisterServerCallback('lds_drugs_system:getDrugsElements', function(playerId, cb)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	local playerInventory = xPlayer.getInventory()
...
RegisterNetEvent('lds_drugs_system:useDrugsIngredients')
AddEventHandler('lds_drugs_system:useDrugsIngredients', function(ingredientsUsed, labId, plyId)
	...
	if drugName then
		if(not isWorking[xPlayer.source]) then
			...
			TriggerClientEvent('lds_drugs_system:isWorking', source, isWorking[xPlayer.source])
		end
		...
		if(isWorking[xPlayer.source]) then
			TriggerEvent('lds_drugs_system:useDrugsIngredients', ingredientsUsed, labId, xPlayer.source)
		else
			return
		end
	else
...
RegisterNetEvent('lds_drugs_system:stopWorking')
AddEventHandler('lds_drugs_system:stopWorking', function()
	isWorking[source] = false
end)

ESX.RegisterServerCallback('lds_drugs_system:getItemLabel', function(playerId, cb, itemName)
...
ESX.RegisterServerCallback('lds_drugs_system:isThereEnoughPoliceToSell', function()
