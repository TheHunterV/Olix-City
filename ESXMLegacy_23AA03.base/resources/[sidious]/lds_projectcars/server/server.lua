...
Citizen.CreateThread(function()
    ...
    if GetResourceKvpString('lds_garage') then
        GlobalState.JobGarage = json.decode(GetResourceKvpString('lds_garage'))
    end
    ...
    local result = SqlFunc(Config.Mysql,'fetchAll','SELECT * FROM lds_projectcars', {})
    ...
    RegisterNetEvent('lds_projectcars:buyshell', function(data)
        local source = source
        ...
        if not Config.MetaInventory and xPlayer.getMoney() >= price then
            local result = SqlFunc(Config.Mysql,'fetchAll','SELECT * FROM lds_projectcars_items WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier})
            if result and result[1] then
                ...
                SqlFunc(Config.Mysql,'execute','UPDATE lds_projectcars_items SET `items` = @items WHERE `identifier` = @identifier', {
                ...
                SqlFunc(Config.Mysql,'execute','INSERT INTO lds_projectcars_items (`identifier`, `items`) VALUES (@identifier, @items)', {
            ...
            xPlayer.removeMoney(price)
            TriggerClientEvent('lds_projectcars:Notify', source, 'success','ProjectCars', Locale[Config.Locale].success_bought_shell)
        end
    end)

    RegisterNetEvent('lds_projectcars:spawnshell', function(data)
        local source = source
        local xPlayer = GetPlayerFromId(source)
        local result = SqlFunc(Config.Mysql,'fetchAll','SELECT * FROM lds_projectcars_items WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier})
        ...
        if result and result[1] and not Config.BuilderJobs[job] then
            ...
            if inv[data.model] then
                ...
                TriggerClientEvent('lds_projectcars:spawnnewproject',source,data.model)
            end
        end
    ...
    RegisterNetEvent('lds_projectcars:buyparts', function(info,val)
        local item = info[1]
        local info = info[2]
        print(item,info)
        ...
        if type(item) == 'table' and GetPaintData(item) then
            ...
            if xPlayer.getMoney() >= (price * val) then
                ...
                TriggerClientEvent('lds_projectcars:Notify', source, 'error','ProjectCars', Locale[Config.Locale].notenoughmoney)
            end
        else
    ...
    RegisterNetEvent('lds_projectcars:updateprojectcars', function(data,plate,props)
        local xPlayer = GetPlayerFromId(source)
    ...
    RegisterNetEvent('lds_projectcars:newproject', function(data)
    local xPlayer = GetPlayerFromId(source)
    ...
    function UpdateProject(data,xPlayer,props)
        ...
        local result = SqlFunc(Config.Mysql,'fetchAll','SELECT * FROM lds_projectcars WHERE TRIM(plate) = @plate', {['@plate'] = plate_})
        ...
        SqlFunc(Config.Mysql,'execute','INSERT INTO lds_projectcars (`plate`, `identifier`, `coord`, `model`, `status`) VALUES (@plate, @identifier, @coord, @model, @status)', {
            ['@plate']   = plate_,
            ['@identifier']   = xPlayer.identifier,
            ['@model']   = data.model,
        ...
        SqlFunc(Config.Mysql,'execute','UPDATE lds_projectcars SET `status` = @status, `paint` = @paint WHERE TRIM(plate) = @plate', {
            ['@plate'] = plate_,
            ['@status'] = status,
            ['@paint'] = projectcars[plate_].paint or '[]'
        })
        TriggerClientEvent('lds_projectcars:updatelocalproject',-1,projectcars[plate_])
    ...
    function AddOwnedVehicles(data,props,xPlayer)
        ...
        GlobalState.ProjectCars = projectcars
        SqlFunc(Config.Mysql,'execute','DELETE FROM lds_projectcars WHERE TRIM(UPPER(plate)) = @plate',{['@plate'] = data.plate})
        TriggerClientEvent('lds_projectcars:updateprojectable',-1,data.plate)
        Wait(1000)
        if xPlayer then
            ...
            TriggerClientEvent('lds_projectcars:spawnfinishproject',xPlayer.source,data,props)
            if GlobalState.GarageInside[xPlayer.identifier] then
                Wait(4000)
    ...
    function RemoveOwnedVehicles(data,props,xPlayer)
        ...
        if xPlayer and Config.MetaInventory then
            ...
            xPlayer.addInventoryItem('vehicle_shell',1,metadata)
            TriggerClientEvent('lds_projectcars:Notify', xPlayer.source, 'success','ProjectCars', Locale[Config.Locale].receive_shell_chop)
        end
        ...
        TriggerClientEvent('lds_projectcars:deletechopped',-1,data.plate)
    ...
    RegisterNetEvent('lds_projectcars:removeitem', function(item,model)
        local xPlayer = GetPlayerFromId(source)
        print(item,model)
    ...
    RegisterNetEvent('lds_projectcars:updatechopcar', function(data)
        local source = source
        local xPlayer = GetPlayerFromId(source)
        local chop = GlobalState.ChopVehicles
        local part = {}
        ...
        if part then
            ...
            TriggerClientEvent('lds_projectcars:updatechopcar',-1,part,data.net,data.part,data.plate,xPlayer.identifier)
        end
        GlobalState.ChopVehicles = chop
    ...
    RegisterNetEvent('lds_projectcars:registerchop', function(data)
        local chop = GlobalState.ChopVehicles
        local newproject = {}
        for k,v in pairs(Config.parts) do
            if k == 'engine' or k == 'transmition' then
        ...
        GlobalState.ChopVehicles = chop
        print("NEW CHOP DATA")
        TriggerClientEvent('lds_projectcars:newchop',-1,data.net,data.plate)
    end)

    RegisterServerCallBack_('lds_projectcars:GenPlate', function (source, cb, prefix)
        cb(GenPlate(prefix))
    end)
    ...
    RegisterNetEvent('lds_projectcars:garage', function(action,data)
        local source = source
        local xPlayer = GetPlayerFromId(source)
        ...
        elseif action == 'buy' and GlobalState.RenterGarage[xPlayer.identifier] then
            TriggerClientEvent('lds_projectcars:Notify', source,'error','ProjectCars', Locale[Config.Locale].alreadygarage..' - GarageID : '..GlobalState.RenterGarage[xPlayer.identifier])
    ...
    RegisterNetEvent('lds_projectcars:changestate', function(plate,job,props,state,model,net)
        local source = source
        plate = string.gsub(plate, '^%s*(.-)%s*$', '%1')
        local xPlayer = GetPlayerFromId(source)
        local jobgarage = {}
        if GetResourceKvpString('lds_garage') == nil then
            jobgarage[job] = {}
            SetResourceKvp('lds_garage',json.encode(jobgarage))
        else
            jobgarage = json.decode(GetResourceKvpString('lds_garage'))
        end
        if state and isPlateOwned(plate) then
            ...
            local tempvehicles = GlobalState.GVehicles or {} -- lds_garage compatibility
        ...
        elseif not state then
            ...
            TriggerClientEvent('lds_projectcars:spawnfinishproject',source,jobgarage[job][plate],jobgarage[job][plate].props)
    ...
    RegisterNetEvent('lds_projectcars:releasejoborder', function(model,data)
        local jobgarage = {}
        local source = source
        local xPlayer = GetPlayerFromId(source)
        local job = xPlayer.job.name
        ...
        if jobgarage[job] then
            ...
            if plate then
                ...
                SetResourceKvp('lds_garage',json.encode(jobgarage))
                GlobalState.JobGarage = jobgarage
                TriggerClientEvent('lds_projectcars:Notify', source, 'success','ProjectCars', Locale[Config.Locale].orderjobrelease..' $'..data.price)
                xPlayer.addMoney(math.random(50000,100000))
                local list = GlobalState.ProjectOrders
                list[job][model] = nil
                GlobalState.ProjectOrders = list
            else
                TriggerClientEvent('lds_projectcars:Notify', source, 'error','ProjectCars', Locale[Config.Locale].cantrelease)
            end
        else
            TriggerClientEvent('lds_projectcars:Notify', source, 'error','ProjectCars', Locale[Config.Locale].novehiclesingarage)
        end
    end)

    RegisterNetEvent('lds_projectcars:requestorderlist', function()
        local list = {}
        local source = source
        local xPlayer = GetPlayerFromId(source)
        local job = xPlayer.job.name
        ...
        GlobalState.ProjectOrders = list
        TriggerClientEvent('lds_projectcars:Notify', source, 'success','ProjectCars', Locale[Config.Locale].orderlistrefresh)
    end)
        ...
        if xPlayer.getGroup() ~= 'user' then
            ...
            for k,v in pairs(projectcars) do
                ...
                if #(pedcoord - project_coord) < 5 then
                    SqlFunc(Config.Mysql,'execute','DELETE FROM lds_projectcars WHERE TRIM(UPPER(plate)) = @plate',{['@plate'] = v.plate})
                    TriggerClientEvent('lds_projectcars:updateprojectable', -1, v.plate)
                    projectcars[k] = nil
                    break
                end
            end
            GlobalState.ProjectCars = projectcars
            TriggerClientEvent('lds_projectcars:Notify', source, 'success','ProjectCars', Locale[Config.Locale].cardeleted)
        else
            TriggerClientEvent('lds_projectcars:Notify', source, 'error','ProjectCars', Locale[Config.Locale].noperms)
        end
    end)
end)