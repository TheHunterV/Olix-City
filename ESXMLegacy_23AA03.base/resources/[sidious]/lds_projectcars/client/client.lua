	...
	if dis < 5 then
		...
		if not IsPedInAnyVehicle(cache.ped) and dis < 5 and chop[plate] == nil and GetModelName(vehicle) and GetVehiclePedIsIn(cache.ped,true) == vehicle  then
			...
			while dis < 5 and GlobalState.ChopVehicles[data.plate] == nil do
				...
				if IsControlJustReleased(0,38) then
					TriggerServerEvent('lds_projectcars:registerchop',data)
					lib.hideTextUI()
				end
			end
			lib.hideTextUI()
		...
		if not IsPedInAnyVehicle(cache.ped) and GlobalState.ChopVehicles[plate] then
			...
			while not chopping and not donechop and dis <= 3.2 and not IsPedWalking(cache.ped) do
				...
				dis = #(mycoord - coord)
				if IsControlJustReleased(0,38) then
					TriggerEvent('lds_projectcars:chop_parts',chop_part,plate,vehicle,coord)
					lib.hideTextUI()
				end
				Wait(0)
			end
...
RegisterNetEvent('lds_projectcars:gotowarehouse', function(brands,name,job,data)
	DoScreenFadeOut(11)
	if name == 'enter' then
...
RegisterNetEvent('lds_projectcars:stockroom', function(data)
	local options = {}
	local cats = {}
    for k,v in pairs(Config.Vehicles) do
		...
		if data[name:lower()] and IsModelInCdimage(GetHashKey(v.model)) then
			if not cats[name] then
				table.insert(options,{
					...
					icon = 'https://cfx-nui-lds_projectcars/html/brands/'..brand..'.png',
					onSelect = function()
						local options = {}
						lib.registerContext({
							id = 'StockRoom_'..name,
			...
			table.insert(cats[name],{
				...
				icon = img,
				onSelect = function()
					TriggerEvent('lds_projectcars:openpartlist',v,name,'StockRoom_')
				end,
				arrow = true,
			})
		end
...
RegisterNetEvent('lds_projectcars:buildermenu', function(data)
	local options = {
		{
			title = 'Create Job',
			...
			icon = img,
			onSelect = function()
				TriggerEvent('lds_projectcars:buildlist',data)
			end,
			arrow = true,
		},
		{
			...
			onSelect = function()
				local options = {
					{
						...
						--icon = img,
						onSelect = function()
							TriggerServerEvent('lds_projectcars:requestorderlist')
						end,
						arrow = true,
					}
				}
					...
					if k == PlayerData.job.name then
						for k,v in pairs(v) do
							...
							table.insert(options,{
								...
								icon = img,
								onSelect = function()
									TriggerServerEvent('lds_projectcars:releasejoborder',k,v)
								end,
								arrow = true,
							})
						end
				...
				TriggerEvent('lds_projectcars:requestorderlist',data)
			end,
			arrow = true,
		}
	}
...
RegisterNetEvent('lds_projectcars:garagemenu', function(data,job)
	local multimenu = {}
	local options = {}
	if not IsPedInAnyVehicle(cache.ped) then
			...
			if k == job then
				for k,v in pairs(v) do
					...
					table.insert(options,{
						...
						onSelect = function()
							TriggerServerEvent('lds_projectcars:changestate',v.props.plate,job,{},false,v.props.model)
						end,
						arrow = true,
					})
				end
		...
		table.insert(options,{
			...
			--icon = img,
			onSelect = function()
				TriggerServerEvent('lds_projectcars:changestate',vplate,job,props,true,{},VehToNet(vehicle))
			end,
			arrow = true,
		})
	end
...
RegisterNetEvent('lds_projectcars:buildlist', function(data)
	local options = {}
	local cats = {}
    for k,v in pairs(Config.Vehicles) do
		local brand = v.brand or 'Imports'
		...
		if data[name:lower()] and IsModelInCdimage(model) then
			...
			table.insert(cats[name],{
				...
				icon = img,
				onSelect = function()
					TriggerServerEvent('lds_projectcars:spawnshell',v)
				end,
				arrow = true,
			})
		end
...
RegisterNetEvent('lds_projectcars:chop_parts', function(part,plate,vehicle,coord)
	...
	if Interaction(part) then
		...
		TriggerServerEvent('lds_projectcars:updatechopcar',data)
	end
end)

RegisterNetEvent('lds_projectcars:updatechopcar', function(part,net,choppped,plate,sender)
...
local localshell = {}
RegisterNetEvent('lds_projectcars:newchop', function(net,plate)
...
RegisterNetEvent('lds_projectcars:deletechopped', function(plate)
...
RegisterNetEvent('lds_projectcars:openautoshop', function(shop)
	local cats = {}
	local options = {}
    for k,v in pairs(shop) do
		...
		if IsModelInCdimage(model) then
			...
			table.insert(cats[name],{
				...
				icon = img,
				onSelect = function()
					TriggerEvent('lds_projectcars:openpartlist',v,name)
				end,
				arrow = true,
			})
		end
...
RegisterNetEvent('lds_projectcars:openshop', function(shop)
	local options = {}
	local cats = {}
    ...
    for k,v in pairs(shop) do
		...
		if IsModelInCdimage(model) then
			...
			table.insert(cats[name],{
				...
				icon = img,
				onSelect = function()
					TriggerServerEvent('lds_projectcars:buyshell',data)
				end,
				arrow = true,
			})
		end
...
RegisterNetEvent('lds_projectcars:openpartlist', function(data,index,warehouse)
	local options = {}
	if data == nil then data = {} data.name = 'Auto Parts' end
	
    for k,v in pairs(Config.parts) do
		local parts = v.label
		local event = 'lds_projectcars:buyparts'
		...
		if inwarehouse == PlayerData.job.name then
			v.price = 0
			string = ''
			event = 'lds_projectcars:useparts'
			var = data.model
		end
        local name = parts

		local img = 'https://cfx-nui-lds_projectcars/html/parts/'..k..'.png'
		...
		table.insert(options,{
			...
			onSelect = function()
				...
				if k == 'paint' then
					TriggerEvent('lds_projectcars:openpaint',data)
				elseif inwarehouse then
					TriggerEvent('lds_projectcars:useparts',k,data.model)
				else
					...
					TriggerEvent('lds_projectcars:openpartlist',data,index)
				end
			end,
		})
    end
...
RegisterNetEvent('lds_projectcars:usepaint', function(color)
	PaintCar(color,getveh())
end)

RegisterNetEvent('lds_projectcars:openpaint', function(data)
	local options = {}
    for k,v in pairs(Config.Paint) do
		
        local name = k:upper()
		
		local img = "https://cfx-nui-lds_projectcars/html/parts/paint_"..k..".png"
		table.insert(options,{
			title = name,
			description = 'Price: '..v.price,
			icon = img,
			onSelect = function()
				...
				if not input then return end
				TriggerServerEvent('lds_projectcars:buyparts',{v,data},input[1])
			end,
		})
    end
...
RegisterNetEvent('lds_projectcars:openblueprints', function(shop)
	local options = {}
    for k,v in pairs(shop) do
		local brand = v.brand or 'Imports'
        local name = brand
		...
		table.insert(cats[name],{
			...
			icon = img,
			onSelect = function()
				TriggerEvent('lds_projectcars:spawnshell',v)
			end,
			arrow = true,
		})
    end
...
RegisterNetEvent('lds_projectcars:useparts', function(part,model)
	Useitem[part](model)
	Wait(500)
end)

RegisterNetEvent('lds_projectcars:Notify', function(type,title,desc)
...
RestoreItem = function(use,model,matched)
	...
	if use and success then
		TriggerServerEvent('lds_projectcars:removeitem',use,model)
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'success','ProjectCars', Locale[Config.Locale].installsuccess)
		else
			Notify(Locale[Config.Locale].installsuccess)
		end
	elseif not matched and Config.MetaInventory then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'warning','ProjectCars', Locale[Config.Locale].partnotmatched)
		else
			Notify(Locale[Config.Locale].partnotmatched)
		end
	else
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'info','ProjectCars', Locale[Config.Locale].partscancel)
		else
...
Useitem['transmition'] = function(model)
	...
	if data and dist < radius then
		if not Config.MetaInventory or Config.MetaInventory and data.model == model then
			SpawnEngine(false)
			TriggerServerEvent('lds_projectcars:removeitem','transmition',model)
			if Config.ldsNotify then
				TriggerEvent('lds_projectcars:Notify', 'success','ProjectCars', Locale[Config.Locale].installsuccess)
			else
				Notify(Locale[Config.Locale].installsuccess)
...
Useitem['engine'] = function(model)
	...
	if data and dist < radius then
		if not Config.MetaInventory or Config.MetaInventory and data.model == model then
			SpawnEngine(true)
			TriggerServerEvent('lds_projectcars:removeitem','engine',model)
			if Config.ldsNotify then
				TriggerEvent('lds_projectcars:Notify', 'success','ProjectCars', Locale[Config.Locale].installsuccess)
			else
				Notify(Locale[Config.Locale].installsuccess)
...
RegisterNetEvent('lds_projectcars:updateprojectable', function(plate)
...
RegisterNetEvent('lds_projectcars:spawnfinishproject', function(data,props)
...
RegisterNetEvent('lds_projectcars:updatelocalproject', function(data)
	local coord = json.decode(data.coord)
	local vehicle = IsAnyVehicleNearPoint(coord.x,coord.y,coord.z,1.1)
	if vehicle then
		local data = {
...
RegisterNetEvent('lds_projectcars:spawnnewproject', function(model)
    DoScreenFadeIn(100)
	local ped = cache.ped
	...
	while spawnedcar ~= nil do
		...
		if IsDisabledControlJustPressed(0, 51) then
			...
			TriggerServerCallback_("lds_projectcars:GenPlate",function(plate)
				...
				TriggerServerEvent('lds_projectcars:newproject',data)
...
SetVehicleUpdate = function(type,index)
	...
	if projectcars[plate] then
		...
		TriggerServerEvent('lds_projectcars:updateprojectcars',projectcars,plate,props)
	end
end
...
InstallDoor = function(id)
	...
	elseif reason == 'seat' then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].install_seat)
		else
			Notify(Locale[Config.Locale].install_seat)
		end
	elseif reason == 'alreadyinstall' then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].already_install)
		else
			Notify(Locale[Config.Locale].already_install)
		end
	else
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].far_away)
		else
...
InstallBonnet = function()
	...
	elseif status.engine == 1 then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].install_engine)
		else
			Notify(Locale[Config.Locale].install_engine)
		end
	elseif status.transmition == 1 then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].install_tranny)
		else
...
InstallWheel = function()
	...
	elseif installbreak then
		if Config.ldsNotify then
			TriggerEvent('lds_projectcars:Notify', 'error','ProjectCars', Locale[Config.Locale].install_brake)
		else
