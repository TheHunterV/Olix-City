...
AddEventHandler('tasks', function(data)
	...
	ESX.TriggerServerCallback("haveitem",function(ret)
        if ret then
			...
			local ret = exports.lds_lockgame:CreateGame(3,{},true)
			if ret then
...
AddEventHandler('openrecipes', function()
	...
	if openmenu then
		TriggerEvent('lds_contextmenu:insertmulti',multimenu,"Open Recipes",false,"<i class='fad fa-hat-chef'></i> Recipes")
