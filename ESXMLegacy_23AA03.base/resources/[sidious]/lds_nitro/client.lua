			...
			while customnitro[plate] ~= nil and customnitro[plate] ~= 'Default' and ent.nitro and ent.nitro.value and ent.nitro.value >= 1 do
				...
				if IsControlPressed(0, 21) then
					...
					if not pressed then
						...
						TriggerServerEvent("lds_nitro:nitro_flame", VehToNet(vehicle ), GetEntityCoords(vehicle ))
					...
					while IsControlPressed(0, 21) and cacheState.nitrovalue >= 0 do
						if smoke and GetVehicleThrottleOffset(vehicle) > 0.05 then
							smoke = false
							TriggerServerEvent("lds_nitro:nitro_flame_stop", VehToNet(vehicle ), GetEntityCoords(vehicle ))
...
lightshit = {}
flametable = {}
RegisterNetEvent("lds_nitro:nitro_flame_stop")
AddEventHandler("lds_nitro:nitro_flame_stop", function(c_veh,coords)
...
local ongoing_nitro = {}
RegisterNetEvent("lds_nitro:nitro_flame")
AddEventHandler("lds_nitro:nitro_flame", function(c_veh,coords)
