...
Citizen.CreateThreadNow(function()
    ...
    for k,v in pairs(Config.Customs) do
        CustomsSQL(Config.Mysql,'execute','INSERT IGNORE  INTO lds_customs (shop) VALUES (@shop)', {
            ['@shop']   = k,
        })
    end
end)

RegisterServerCallBack_('lds_customs:getinventory', function (source, cb, id, share)
    local source = source
    ...
    local result = CustomsSQL(Config.Mysql,'fetchAll','SELECT inventory FROM lds_customs WHERE shop = @shop', {
        ['@shop'] = id
    })
    local inventory = {}
...
RegisterServerCallBack_('lds_customs:itemavailable', function (source, cb, id, item, share)
    local source = source
    local xPlayer = GetPlayerFromId(source)
    local identifier = xPlayer.identifier
    if share then
    ...
    if json.decode(result[1].inventory) then
        ...
        if inventory[item] ~= nil and inventory[item] > 0 then
            ...
            CustomsSQL(Config.Mysql,'execute','UPDATE lds_customs SET inventory = @inventory WHERE shop = @shop', {
...
RegisterServerEvent('lds_customs:storemod')
AddEventHandler('lds_customs:storemod', function(id,mod,lvl,newprop,share,save,savepartsonly)
    local src = source  
    ...
    TriggerClientEvent('lds_notify:Notify', src, 'success','Garage', 'You Successfully store the parts ('..mod.name..')')
end)
...
function Jobmoney(job,xPlayer)
    ...
    local job = job
    local count = 0
    if Config.Uselds_jobs then
        value = exports.lds_jobs:JobMoney(job).money
    else
...
RegisterServerCallBack_('lds_customs:pay', function (source, cb, t, shop, vclass)
    local src = source  
    local xPlayer = GetPlayerFromId(src)
    local identifier = xPlayer.identifier
    local prop = t.prop
    ...
    if not menu and not Config.FreeUpgradeToClass[vclass] and not Config.JobPermissionAll and xPlayer.getMoney() >= t.cost or not menu and not Config.FreeUpgradeToClass[vclass] and Config.JobPermissionAll and Config.Customs[shop].job == xPlayer.job.name and Jobmoney(xPlayer.job.name,xPlayer) >= t.cost or menu then
        ...
        if result[1] or menu then
            ...
            elseif Config.JobPermissionAll and not Config.Uselds_jobs and not menu then -- job owned without lds_jobs
                xPlayer.removeMoney(cost) -- replace it with your job money
            end
            if menu then
                TriggerClientEvent('lds_notify:Notify', src, 'success','Customs', 'MENU - Upgrade has been Installed')
            else
                TriggerClientEvent('lds_notify:Notify', src, 'success','Customs', 'Payment Success - Upgrade has been Installed')
            end
            if shop and not Config.JobPermissionAll and not menu then
                if Config.Uselds_jobs then
                    addmoney = exports.lds_jobs:addMoney(tonumber(t.cost),Config.Customs[shop].job,source,'money',true)
                else
            ...
            elseif shop and Config.JobPermissionAll and Config.Customs[shop].job == xPlayer.job.name and not menu then
                if Config.Uselds_jobs then
                    removemoney = exports.lds_jobs:removeMoney(tonumber(t.cost),Config.Customs[shop].job,source,'money',true)
                else
        ...
        elseif not Config.OwnedVehiclesOnly or menu then
            ...
            if shop and not Config.JobPermissionAll and xPlayer.getMoney() >= tonumber(t.cost) or Config.JobPermissionAll and Config.Customs[shop].job == xPlayer.job.name and Jobmoney(xPlayer.job.name,xPlayer) >= tonumber(t.cost) then
                ...
                TriggerClientEvent('lds_notify:Notify', src, 'error','Customs', 'Not Enough Money Cabron')
                cb(false)
            end
        else
            TriggerClientEvent('lds_notify:Notify', src, 'error','Customs', 'Vehicle is not Owned')
            cb(false)
        end
    elseif Config.FreeUpgradeToClass[vclass] then
        TriggerClientEvent('lds_notify:Notify', src, 'success','Customs', 'FREE Upgrade has been Installed')
        local result = CustomsSQL(Config.Mysql,'fetchAll','SELECT * FROM '..vehicletable..' WHERE UPPER(plate) = @plate', {
            ['@plate'] = prop.plate:upper()
        })
        if result[1] then
...
RegisterServerCallBack_('lds_customs:repair', function (source, cb, shop)
    local src = source  
    local xPlayer = GetPlayerFromId(src)
    local jobmoney = 0
    if xPlayer.getMoney() >= Config.RepairCost and not menu then
        if Config.Uselds_jobs and Config.Customs[shop].job ~= xPlayer.job.name then -- job permission access is free repair
            addmoney = exports.lds_jobs:addMoney(tonumber(Config.RepairCost),Config.Customs[shop].job,source,'money',true)
        end
...
local inshop = {}
RegisterServerCallBack_('lds_customs:getmoney', function (source, cb, net, props)
    local src = source  
    local xPlayer = GetPlayerFromId(src)
    inshop[source] = {net = net , props = props}
    local money = 0
    if Config.Uselds_jobs and Config.JobPermissionAll then
...
AddEventHandler('playerDropped', function(reason)
	for k,v in pairs(inshop) do
        if tonumber(source) == tonumber(k) then
            TriggerClientEvent('lds_customs:restoremod',-1 , v.net, v.props)
            --DeleteEntity(v)
        end
    end
end)

RegisterServerEvent('lds_customs:leaveshop')
AddEventHandler('lds_customs:leaveshop', function()
	inshop[source] = nil
end)
...
RegisterServerEvent('lds_customs:syncdel')
AddEventHandler('lds_customs:syncdel', function(net)
    local source = source
...
local customengine = {}
RegisterServerEvent('lds_customs:custom_engine')
AddEventHandler('lds_customs:custom_engine', function(netid,plate,model)
    ...
    TriggerClientEvent('lds_customs:receivenetworkid',-1,GetVehicleNetWorkIdByPlate(plate,source),model)
    SetResourceKvp('engine',json.encode(customengine))
end)

local customturbo = {}
RegisterServerEvent('lds_customs:custom_turbo')
AddEventHandler('lds_customs:custom_turbo', function(plate,turbo)
    customturbo[plate] = turbo
    if turbo == 'Default' then customturbo[plate] = nil end
    TriggerClientEvent('lds_customs:receiveturboupgrade',-1,customturbo)
    SetResourceKvp('turbo',json.encode(customturbo))
end)

local customtire = {}
RegisterServerEvent('lds_customs:custom_tire')
AddEventHandler('lds_customs:custom_tire', function(plate,tire)
    customtire[plate] = tire
    local source = source
    if tire == 'Default' then customtire[plate] = nil end
    Wait(1500) -- added wait for other garage script compatibility (setprop before ped is in vehicle)
    TriggerClientEvent('lds_customs:custom_tire',-1,customtire,GetVehicleNetWorkIdByPlate(plate,source),tire)
    SetResourceKvp('tire',json.encode(customtire))
end)

RegisterServerEvent('lds_customs:soundsync')
AddEventHandler('lds_customs:soundsync', function(table)
    TriggerClientEvent('lds_customs:soundsync',-1,table)
end)

RegisterServerEvent('lds_customs:loaded')
AddEventHandler('lds_customs:loaded', function()
    local source = source
    TriggerClientEvent('lds_customs:receivedata',source,customturbo,customengine,vehiclesname)
end)
...
RegisterCommand('freecustoms', function (source, args)
    ...
    if Config.framework == 'ESX' and playerGroup == "superadmin" or playerGroup == "mod" or playerGroup == "admin" or Config.framework == 'QBCORE' and playerGroup then
        TriggerClientEvent('lds_customs:openmenu',source, true)
    end
end)