...
RegisterNUICallback('repair', function(data, cb)
    ...
    TriggerServerCallback_("lds_customs:repair",function(ret)
        if ret then
            if Config.Uselds_progressbar then
                repair = exports.lds_progressbar:CreateProgressBar(25,'<i class="fas fa-tools"></i>')
            end
            ...
            TriggerEvent('lds_notify:Notify', 'error','Customs', 'Not enough money cabron $'..Config.RepairCost..' Required')
            cb(false)
        end
    end,currentprivate)
end)
...
RegisterNUICallback('Close', function(data, cb)
    ...
    TriggerServerEvent('lds_customs:leaveshop')
end)
...
RegisterNUICallback('pay', function(data, cb)
    ...
    TriggerServerCallback_("lds_customs:pay",function(cb)
        if cb then
            DisplayHud(true)
            SetNuiFocus(false, false)
            SetNuiFocusKeepInput(false)  
