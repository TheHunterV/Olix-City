RegisterNetEvent('lds_customs:ingarage')
AddEventHandler('lds_customs:ingarage', function(garage,garage_id)
        ...
        while insidegarage do
            for k,v in pairs(Config.Customs) do
                ...
                if Config.InteractiveFeature['paintmenu'] and distance < 30 and ShopPermmision(currentprivate,'paintmenu') and Config.showmarker then
                    DrawMarkerInput(vector3(v.paintmenu.coord.x,v.paintmenu.coord.y,v.paintmenu.coord.z),'Spray Paint Menu','lds_customs:openpaintmenu',false,'spray_paint')
                end
                if Config.InteractiveFeature['paintmenu'] and distance < 3 and ShopPermmision(currentprivate,'paintmenu') and Config.usePopui then
                    local table = {
                        ['key'] = 'E', -- key
                        ['event'] = 'lds_customs:openpaintmenu',
                    ...
                    for k,v in pairs(v.mod) do
                        ...
                        if Config.JobPermissionAll and ShopPermmision(currentprivate) or not Config.JobPermissionAll then
                            if distance < 30 and invehicle and Config.showmarker then
                                DrawMarkerInput(vector3(v.coord.x,v.coord.y,v.coord.z),'Upgrade Menu','lds_customs:openmenu',false,'mod_'..k)
                            end
                            if distance < 3 and invehicle and Config.usePopui then
                                local table = {
                                    ['key'] = 'E', -- key
                                    ['event'] = 'lds_customs:openmenu',
                ...
                if Config.InteractiveFeature['stockroom'] and distance < 30 and ShopPermmision(currentprivate,'stockroom') and Config.showmarker then
                    DrawMarkerInput(vector3(v.stockroom.coord.x,v.stockroom.coord.y,v.stockroom.coord.z),'Stock Room','lds_customs:openstockroom',false,'stock_inventory',k)
                end
                if Config.InteractiveFeature['stockroom'] and distance < 3 and ShopPermmision(currentprivate,'stockroom') and Config.usePopui then
                    local table = {
                        ['key'] = 'E', -- key
                        ['event'] = 'lds_customs:openstockroom',
                    ...
                    CreateThread(function()
                        while nearveh ~= 0 do
                            ...
                            if IsControlPressed(0,38) then
                                TriggerEvent('lds_customs:vehiclemod',nearveh)
                                Wait(100)
                                break
                            end
                            Wait(4)                            
            ...
            if Config.InteractiveFeature['garage_inventory'] and inventorydis < 10 and not carrymode and not carrymod and ShopPermmision(currentprivate,'garage_inventory') and Config.showmarker then
                DrawMarkerInput(vector3(inv.coord.x,inv.coord.y,inv.coord.z),'Parts Inventory','lds_customs:openinventory',false,'parts_inventory',currentprivate)
            end
            if Config.InteractiveFeature['garage_inventory'] and inventorydis < 2 and not carrymode and not carrymod and ShopPermmision(currentprivate,'garage_inventory') and Config.usePopui then
                local table = {
                    ['key'] = 'E', -- key
                    ['event'] = 'lds_customs:openinventory',
            ...
            elseif Config.InteractiveFeature['garage_inventory'] and not carrymode and carrymod and inventorydis < 2 and ShopPermmision(currentprivate,'garage_inventory') then
                if inventorydis < 2 and ShopPermmision(currentprivate,'garage_inventory') and Config.showmarker then
                    DrawMarkerInput(vector3(inv.coord.x,inv.coord.y,inv.coord.z),'Press [E] Store','lds_customs:storemod',false,'store_mod',{currentprivate,Config.VehicleMod[tostore[1]],tostore[2],false,false,true},true)
                end
                if Config.usePopui then
                    local table = {
                        ['key'] = 'E', -- key
                        ['event'] = 'lds_customs:storemod',
                        ['title'] = 'Press [E] Store',
                        ['server_event'] = false, -- server event or client
                        ['unpack_arg'] = true, -- send args as unpack 1,2,3,4 order
                        ['fa'] = '<i class="fas fa-car"></i>',
...
RegisterNetEvent('lds_customs:openinventory')
AddEventHandler('lds_customs:openinventory', function(current)
    local multimenu = {}
    local firstmenu = {}
    local openmenu = false
    TriggerServerCallback_("lds_customs:getinventory",function(inventory)
        ...
        if openmenu then
            TriggerEvent('lds_contextmenu:insertmulti',multimenu,"Vehicle Parts",false,"<i class='fad fa-inventory'></i> Mod Inventory")
            TriggerEvent('lds_contextmenu:show')
        else
            TriggerEvent('lds_notify:Notify', 'error','Customs', 'Inventory is Empty')
        end
    end,current,activeshare)
end)

RegisterNetEvent('lds_customs:openstockroom')
AddEventHandler('lds_customs:openstockroom', function(current)
    multimenu = {}
    local firstmenu = {}
    openmenu = false
                    ...
                    if max >= 0 then
                        ...
                        for i = 0, max do
                            ...
                            if multimenu[vehicle][v.label].main_fa == nil then
                                multimenu[vehicle][v.label].main_fa = '<img style="height: auto;margin-left: -20px;margin-top: -10px;position: relative;max-width: 35px;float: left;" src="https://cfx-nui-lds_customs/html/img/'..k..'.svg">'
                            end
                            multimenu[vehicle][v.label][i] = {
                                ['title'] = label,
                                ['fa'] = '<i class="fad fa-car"></i>',
                                ['type'] = 'event', -- event / export
                                ['content'] = 'lds_customs:getmodfromstockroom',
    ...
    for k,v in pairs(vehicleinarea) do
        ...
        if localmultimenu[name] == nil then
            ...
            localmultimenu[name][k] = {
                ...
                ['fa'] = '<i class="fad fa-car"></i>',
                ['type'] = 'event', -- event / export
                ['content'] = 'lds_customs:partlist',
                ['variables'] = {server = false, send_entity = false, onclickcloseui = true, custom_arg = {k}, arg_unpack = true},
            }
        end
    end
    if openmenu then
        TriggerEvent('lds_contextmenu:insertmulti',localmultimenu,"Vehicle Parts",false,"<i class='fad fa-container-storage'></i> Stock Room")
        TriggerEvent('lds_contextmenu:show')
    end
end)

RegisterNetEvent('lds_customs:partlist')
AddEventHandler('lds_customs:partlist', function(vehicle)
    local max
    local firstmenu = {}
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
        SetModable(vehicle)
        if openmenu then
            TriggerEvent('lds_contextmenu:insertmulti',multimenu[vehicle],"Vehicle Parts",false,"<i class='fas fa-cog'></i> Part List")
            Wait(100)
            TriggerEvent('lds_contextmenu:show')
        end
    end
...
RegisterNetEvent('lds_customs:getmodfromstockroom')
AddEventHandler('lds_customs:getmodfromstockroom', function(index,lvl,name,vehicle)
    carrymod = true
    local k = name..'-'..lvl
...
RegisterNetEvent('lds_customs:storemod')
AddEventHandler('lds_customs:storemod', function(current,mod,lvl,newprop,save,saveprop)
    carrymode = false
    carrymod = false
    TriggerServerEvent('lds_customs:syncdel',NetworkGetNetworkIdFromEntity(object))
    ClearPedTasks(PlayerPedId())
    TriggerServerEvent('lds_customs:storemod',current,mod,lvl,newprop,activeshare,save,saveprop)
end)

RegisterNetEvent('lds_customs:installmod')
AddEventHandler('lds_customs:installmod', function(index,lvl,k,vehicle,mod)
    ...
    if tonumber(lvl) <= max then
        ...
        TriggerServerEvent('lds_customs:storemod',currentprivate,mod,lvl,newprop,activeshare,true)
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('lds_customs:syncdel',NetworkGetNetworkIdFromEntity(object))
    else
        TriggerEvent('lds_notify:Notify', 'error','Garage', 'This Parts is not compatible with this vehicle')
    end
end)

RegisterNetEvent('lds_customs:getmod')
AddEventHandler('lds_customs:getmod', function(index,lvl,k)
    TriggerServerCallback_("lds_customs:itemavailable",function(inventory)
        if inventory then
            carrymod = true
            CarryMod("anim@heists@box_carry@","idle",Config.VehicleMod[index].prop or 'hei_prop_heist_box',50,28422)
            tostore = {index,lvl,k,false,Config.VehicleMod[index]}
            ...
            TriggerEvent('lds_notify:Notify', 'error','Garage', 'This Parts Does not exist')
        end
    end,currentprivate,k,activeshare)
end)

RegisterNetEvent('lds_customs:removevehiclemod')
AddEventHandler('lds_customs:removevehiclemod', function(mod,lvl,vehicle)
    if mod ~= nil and GetVehicleMod(vehicle,tonumber(mod.index)) + 1 >= lvl then
        ...
        while carrymode do
            ...
            if distance < 3 and ShopPermmision(currentprivate,'garage_inventory') and Config.showmarker then
                DrawMarkerInput(vector3(vec.coord.x,vec.coord.y,vec.coord.z),'Press [E] Store','lds_customs:storemod',false,'store_mod',{currentprivate,mod,lvl,newprop},true)
            end
...
RegisterNetEvent('lds_customs:vehiclemod')
AddEventHandler('lds_customs:vehiclemod', function(vehicle)
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
        local multimenu = {}
        local firstmenu = {}
        local openmenu = false
        for i = 0, 49 do
            ...
            if Config.VehicleMod[i] ~= nil then
                ...
                if GetVehicleMod(vehicle,mod.index) + 1 > 0 then
                    ...
                    multimenu[mod.type:upper()].main_fa = '<img style="height: auto;margin-left: -20px;margin-top: -10px;position: relative;max-width: 35px;float: left;" src="https://cfx-nui-lds_customs/html/img/'..i..'.svg">'
                    multimenu[mod.type:upper()][mod.label:upper()..' '..GetVehicleMod(vehicle,i) + 1] = {
                        ['title'] = mod.label..' '..GetVehicleMod(vehicle,i) + 1,
                        ['fa'] = '<img style="height: auto;position: absolute;max-width: 30px;left:5%;top:25%;" src="https://cfx-nui-lds_customs/html/img/'..i..'.svg">',
                        ['type'] = 'event', -- event / export
                        ['content'] = 'lds_customs:removevehiclemod',
        ...
        if openmenu then
            TriggerEvent('lds_contextmenu:insertmulti',multimenu,"Vehicle Parts",false,"Vehicle Mods")
            TriggerEvent('lds_contextmenu:show')
        else
            TriggerEvent('lds_notify:Notify', 'error','Customs', 'Vehicle is Stock')
        end
    end
end)

RegisterNetEvent('lds_customs:soundsync')
AddEventHandler('lds_customs:soundsync', function(table)
...
RegisterNetEvent('lds_customs:receivenetworkid')
AddEventHandler('lds_customs:receivenetworkid', function(net,model)
...
RegisterNetEvent('lds_customs:receiveturboupgrade')
AddEventHandler('lds_customs:receiveturboupgrade', function(turbos)
    customturbo = turbos
end)

RegisterNetEvent('lds_customs:custom_tire')
AddEventHandler('lds_customs:custom_tire', function(tires,net,tire)
    customtire = tires
...
RegisterNetEvent('lds_customs:openmenu')
AddEventHandler('lds_customs:openmenu', function(menu)
    local custom = {}
    local vehicle = GetVehiclePedIsIn(PlayerPedId())
    if vehicle == 0 then
        vehicle = GetVehiclePedIsIn(PlayerPedId(),true)
    ...
    for k,v in pairs(Config.Customs) do
        ...
        if distance < v.radius or menu then
            TriggerServerCallback_("lds_customs:getmoney",function(money)
...
RegisterNetEvent('lds_customs:restoremod')
AddEventHandler('lds_customs:restoremod', function(net,prop)
    local ent = NetworkGetEntityFromNetworkId(net)
    SetModable(ent)
...
RegisterNetEvent('lds_customs:paint')
AddEventHandler('lds_customs:paint', function(color,spray)
    if not spray then
        spraying = true
        tospray = true
        ...
        TriggerServerEvent('lds_customs:syncdel',NetworkGetNetworkIdFromEntity(spraycan))
    else
        tospray = false
    end
end)

RegisterNetEvent('lds_customs:openpaintmenu')
AddEventHandler('lds_customs:openpaintmenu', function(garage,garage_id)
    local localmultimenu = {}
    local openmenu = false
    for name,color in pairs(Config.Pilox) do
        ...
        if localmultimenu[name] == nil then
            ...
            localmultimenu[name][name] = {
                ...
                ['fa'] = '<i class="fad fa-car"></i>',
                ['type'] = 'event', -- event / export
                ['content'] = 'lds_customs:paint',
                ['variables'] = {server = false, send_entity = false, onclickcloseui = true, custom_arg = {name}, arg_unpack = true},
            }
        end
    end
    if openmenu then
        TriggerEvent('lds_contextmenu:insertmulti',localmultimenu,"Vehicle Parts",false,"<i class='fas fa-spray-can'></i> Spray Paints")
        TriggerEvent('lds_contextmenu:show')
    end
end)
...
RegisterNetEvent('lds_customs:receivedata')
AddEventHandler('lds_customs:receivedata', function(turbo,engine,vehicleprice)
	netids = engine
    customturbo = turbo
