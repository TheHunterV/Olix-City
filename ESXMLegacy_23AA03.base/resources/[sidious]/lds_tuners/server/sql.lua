...
sql = setmetatable({},{
	__call = function(self)
		...
		self.insert = function(column, data, plate)
			...
			return MySQL.insert.await(str:format('lds_tuner',column,'?','?'),{data,plate})
		end

		self.update = function(column, where, string, data)
			local str = 'UPDATE %s SET %s = ? WHERE %s = ?'
			return MySQL.update(str:format('lds_tuner',column,where),{data,string})
		end

		self.query = function(column, where, string)
			local str = 'SELECT %s FROM %s WHERE %s = ?'
			return MySQL.query.await(str:format(column,'lds_tuner',where),{string})
		end

		self.fetchAll = function()
			local str = 'SELECT * FROM lds_tuner'
		...
		self.save = function(column, where, string, data)
			...
			local success, result = pcall(MySQL.scalar.await, str:format('lds_tuner',where),{string})
		...
		self.updateall = function(data,plate)
			...
			local str = 'SELECT 1 FROM %s WHERE %s = ?'
			local success, result = pcall(MySQL.scalar.await, str:format('lds_tuner','plate'),{plate})
			if success and result then
				local str = 'UPDATE %s SET %s = ?, %s = ?, %s = ?, %s = ? WHERE %s = ?'
				MySQL.update(str:format('lds_tuner','vehiclestats','defaulthandling','vehicleupgrades','mileages','plate'),{data.vehiclestats,data.defaulthandling,data.vehicleupgrades,data.mileages,plate})
			else
				local str = 'INSERT INTO %s (%s, %s, %s, %s, %s) VALUES(?, ?, ?, ?, ?)'
				MySQL.insert.await(str:format('lds_tuner','vehiclestats','defaulthandling','vehicleupgrades','mileages','plate'),{data.vehiclestats,data.defaulthandling,data.vehicleupgrades,data.mileages,plate})
			end
...
Citizen.CreateThreadNow(function()
	...
	local success, result = pcall(MySQL.scalar.await, 'SELECT 1 FROM lds_tuner')
	if not success then
		MySQL.query.await([[CREATE TABLE `lds_tuner` (
	...
	pcall(MySQL.query.await, 'ALTER TABLE `lds_tuner` CHANGE COLUMN `advancedflags` `advancedflags` LONGTEXT NULL') -- temp
	local success, result = pcall(MySQL.scalar.await,'SELECT `nodegrade` FROM lds_tuner') -- check if nodegrade column is exist
	if not success then
		pcall(MySQL.query.await, 'ALTER TABLE lds_tuner ADD COLUMN `nodegrade` int DEFAULT 0')
	end
end)
