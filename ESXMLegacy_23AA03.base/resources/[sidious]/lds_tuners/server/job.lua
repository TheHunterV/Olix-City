...
GetJobMoney = function(job)
    local value = 0
    if GetResourceState('lds_jobs') == 'started' then
		return exports.lds_jobs:JobMoney(job).money
...
lib.callback.register('lds_tuners:getJobMoney', function(src,job)
...
RemoveJobMoney = function(job,amount)
	if GetResourceState('lds_jobs') == 'started' then
		return exports.lds_jobs:removeMoney(amount,job,nil,'money',true)
    elseif GetResourceState('es_extended') == 'started' then
        TriggerEvent('esx_addonaccount:getSharedAccount', 'society_'..job, function(account)
