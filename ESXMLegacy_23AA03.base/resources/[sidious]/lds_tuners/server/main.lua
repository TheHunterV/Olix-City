...
lib.callback.register('lds_tuners:CheckDyno', function(src,dynamometer,index)
...
lib.callback.register('lds_tuners:vehiclestats', function(src, plate) -- only the efficient way to send data to client. normaly people will just fetch sql every time player goes into vehicle. which is not performant.
...
lib.callback.register('lds_tuners:Tune', function(src,data)
...
lib.callback.register('lds_tuners:checkitem', function(src,item,isShop,required)
...
lib.callback.register('lds_tuners:OldEngine', function(src,name,engine,plate,net)
	local metadata = {}
	local vehicle = NetworkGetEntityFromNetworkId(net)
...
lib.callback.register('lds_tuners:GetEngineStorage', function(src,stash)
	return GetInventoryItems(stash, 'slots', 'enginegago')
end)

lib.callback.register('lds_tuners:RemoveEngineStorage', function(src,data)
	RemoveInventoryItem(data.stash, data.name, 1, data.metadata, data.slot)
end)

lib.callback.register('lds_tuners:Craft', function(src,slots,requiredata,item,engine)
	local success = false
...
lib.callback.register('lds_tuners:RepairPart', function(src,percent,noMetadata)
	local src = src
...
lib.addCommand('sandboxmode', {
    ...
    TriggerClientEvent('lds_tuners:SandBoxmode',source)
end)
