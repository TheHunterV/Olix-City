	...
	lib.registerMenu({
		...
		for k,v in pairs(config.engineupgrades) do
			if v.category == args:lower() then
				local hasitem = lib.callback.await('lds_tuners:checkitem',false,v.item)
...
Options = function(data,shop,job)
	...
	elseif data.nitro then
		local nitros = exports.lds_nitro:nitros()
	...
	if hasmenu then
		local jobmoney = lib.callback.await('lds_tuners:getJobMoney',false,PlayerData?.job?.name)
		lib.registerMenu({
			id = 'upgrade_options',
			...
			onCheck = function(selected, checked, args)
				...
				local hasitem = lib.callback.await('lds_tuners:checkitem',false,item)
				if config.freeupgrade or hasitem then
					ItemFunction(vehicle,{
						name = item,
						label = data.label
			...
			if type(args) == 'table' and args.tune then
				if args.profile == 'new' then
					...
					if input and input[1] then
						...
						lib.callback.await('lds_tuners:Tune',false,{vehicle = NetworkGetNetworkIdFromEntity(vehicle) ,profile = input[1], tune = {acceleration = input[2], topspeed = input[5], engineresponse = input[3], gear_response = input[4], boostpergear = boostpergear, gear_ratio = gear_ratio}})
						Wait(200)
						...
						vehiclestats, vehicletires, mileages, ecu = lib.callback.await('lds_tuners:vehiclestats', 0, plate)
					else
						lib.notify({
					...
					if input and input[1] then
						...
						lib.callback.await('lds_tuners:Tune',false,{vehicle = NetworkGetNetworkIdFromEntity(vehicle) ,profile = args.profile, tune = {acceleration = input[1], topspeed = input[4], engineresponse = input[2], gear_response = input[3], boostpergear = boostpergear, gear_ratio = gear_ratio}})
						Wait(200)
						HandleEngineDegration(vehicle,Entity(vehicle).state,plate)
						local plate = string.gsub(GetVehicleNumberPlateText(GetVehiclePedIsIn(cache.ped)), '^%s*(.-)%s*$', '%1'):upper()
						vehiclestats, vehicletires, mileages, ecu = lib.callback.await('lds_tuners:vehiclestats', 0, plate)
				...
				if name == 'repairparts' then
					...
					local success = lib.callback.await('lds_tuners:RepairPart',false,percent,ismetadataSupport == false)
					...
					hasitem = lib.callback.await('lds_tuners:checkitem',false,item)
				end
				if config.freeupgrade or hasitem then
...
CheckVehicle = function(menu,shop)
	...
	if menu then
		...
		if GetResourceState('lds_turbo') == 'started' then
		...
		if GetResourceState('lds_nitro') == 'started' then
		...
		if GetResourceState('lds_engine') == 'started' then
			local engine = ent['engine'] or 'Default'
			table.insert(options,{icon = imagepath..'engine.png' , label = 'Engine (Locals) (current: '..engine..')', description = ' Installed Engines', colorScheme = 'black',  args = {localengine = true, label = 'Engine Swap', value = exports.lds_engine:Engines().Locals}})
			table.insert(options,{icon = imagepath..'engine.png' , label = 'Engine (Customs) (current: '..engine..')', description = ' Installed Engines', colorScheme = 'black',  args = {customengine = true, label = 'Engine Swap', value = exports.lds_engine:Engines().Custom}})
		end
	end
...
TuningMenu = function()
	...
	lib.registerMenu({
		...
		if not config.sandboxmode then
			lib.callback.await('lds_tuners:Tune',false,{vehicle = NetworkGetNetworkIdFromEntity(vehicle) ,profile = activeprofile.profile, tune = activeprofile})
		else
			for k,v in ipairs(args.attributes) do
...
ContextMenuOptions = function(stash,entity,vehicle)
	Wait(1000)
	if GetResourceState('lds_engine') ~= 'started' then return end
	...
	local ent = Entity(vehicle).state
	local items = lib.callback.await('lds_tuners:GetEngineStorage',false,stash)
	local options = {}
	for k,v in pairs(items) do
		...
		table.insert(options,{
			...
			onSelect = function()
				...
				TriggerServerEvent('lds_engine:EngineSwap',NetworkGetNetworkIdFromEntity(vehicle),engine)
				for k,v in pairs(v.metadata) do
					if type(v) == 'table' then
				...
				lib.callback.await('lds_tuners:RemoveEngineStorage',false,{stash = stash, name = v.name, slot = v.slot, metadata = v.metadata})
		...
		table.insert(options,{
			...
			onSelect = function()
				...
				if hasitems then
					...
					local success = lib.callback.await('lds_tuners:Craft',false,slots,requiredata,item,craft == 'engine' and item)
					if success then
