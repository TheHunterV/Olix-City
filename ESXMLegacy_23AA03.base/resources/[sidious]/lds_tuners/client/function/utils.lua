...
SaveStateFlags = function(vehicle,state,data) -- handle Advanced Flags Handling
	local export = exports.lds_tuners
...
GetVehicleServerStates = function(plate) -- fetch only the current vehicle data
	...
	vehiclestats, vehicletires, mileages, ecu = lib.callback.await('lds_tuners:vehiclestats', 0, plate) -- temporary work around to bypass statebag size limits.
	return true
end
...
SetDefaultHandling = function(vehicle,handling) -- setter from other resource ex. lds_engine
	local ent = Entity(vehicle).state
	if ent.engine then
