...
RetrieveOldEngine = function(vehicle,engine)
	local ent = Entity(vehicle).state
	local engines = exports.lds_engine:Engines().Locals
	local custom_engine = exports.lds_engine:Engines().Custom
	...
	lib.callback.await('lds_tuners:OldEngine',false,label,model,plate,NetworkGetNetworkIdFromEntity(vehicle))
	ent:set('currentengine',engine,true)
end

RegisterNetEvent('lds_engine:OnEngineChange', function(engine) -- repair current parts when installing new engines
