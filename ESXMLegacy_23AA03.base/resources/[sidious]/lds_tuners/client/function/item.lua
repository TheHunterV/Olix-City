...
ItemFunction = function(vehicle,data,menu) -- item use handler
	...
	elseif isturbo then
		TriggerServerEvent('lds_turbo:AddTurbo',NetworkGetNetworkIdFromEntity(vehicle),item)
	elseif isnitro then
		TriggerServerEvent('lds_nitro:AddNitro',NetworkGetNetworkIdFromEntity(vehicle),item)
	elseif data.engine then
		TriggerServerEvent('lds_engine:EngineSwap',NetworkGetNetworkIdFromEntity(vehicle),item)
	elseif ECU then
		local boostpergear = {}
		for i = 1, GetVehicleHighGear(vehicle) do
			boostpergear[i] = 1.0
		end
		lib.callback.await('lds_tuners:Tune',false,{vehicle = NetworkGetNetworkIdFromEntity(vehicle) ,profile = 'Default', tune = {acceleration = 1.0, topspeed = 1.0, engineresponse = 1.0, gear_response = 1.0, boostpergear = boostpergear}})
		Wait(1000)
