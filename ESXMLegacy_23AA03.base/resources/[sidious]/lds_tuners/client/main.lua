...
OnVehicle = function(value)
	...
	local turbo = ent.turbo?.turbo -- lds_turbo states bag
	local ecu_state = ecu
	local turbopower = 1.0
	local turboinstall = GetResourceState('lds_turbo') == 'started'
	if value then
		...
		Citizen.CreateThreadNow(function()
			...
			while invehicle do
				...
				if ecu and ecu[gear-1] and not indyno and turboinstall then
					exports.lds_turbo:BoostPerGear(ecu[gear-1] or 1.0)
				end
	...
	Citizen.CreateThreadNow(function()
        ...
        while invehicle do
			...
			if rpm > 0.5 then -- start degrading states if its above 0.5 RPM
				...
				local update = false
				local nitro = ent.nitroenable -- lds_nitro states bag if nitro is being used
...
Citizen.CreateThreadNow(function()
	...
	local isturbostarted = GetResourceState('lds_turbo') == 'started'
	if isturbostarted then
		turboconfig = exports.lds_turbo:turbos()
	end
