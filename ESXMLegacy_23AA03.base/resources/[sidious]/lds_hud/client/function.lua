...
-----------------------------------------------------------------------------------------------------------------------------------------
-- lds HUD function Hud: https://github.com/ldszu/lds_hud
-----------------------------------------------------------------------------------------------------------------------------------------
...
function Hud:UpdateStatus(export,vitals)
	...
	if export and config.framework == 'QBCORE' and self.esx_status ~= nil or export and config.framework ~= 'QBCORE' and self.esx_status ~= nil then
		if not self.esx_status then
			self.vitals = exports['lds_status']:GetStatus(self.statuses)
		end
...
local vtable = {}
Creation(function()
	ldsNuiCallback('getvehicledata', function(data, cb)
...
function Hud:lds_Function(func)
	local f = {}
	setmetatable(f, {
		__close = func
	})
