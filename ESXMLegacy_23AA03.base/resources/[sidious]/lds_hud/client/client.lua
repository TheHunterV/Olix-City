...
local cdch = 0
RegisterNetEvent("lds_hud:SetVoiceData", function(mode,val)
	if mode == 'radio' then
...
RegisterNetEvent('lds_hud:charslot', function(charid)
	Hud.charslot = charid
...
CreateThread(function()
	...
	if config.enablestatus then
		...
		local vitals = {}
		AddEventHandler("esx_status:onTick", function(vitals) -- use lds_status
			vitals = vitals
			if vitals[1] ~= nil then -- esx status (if index int is not nil, its a normal esx_status) else its lds_status
				...
				for k,v in ipairs(vitals) do
					if config.statusordering[v.name] ~= nil then
						vitals[v.name] = v.val -- populate the data from esx_status and convert to lds_status format
						vitals[k] = nil
					else
						vitals[k] = nil
					end
