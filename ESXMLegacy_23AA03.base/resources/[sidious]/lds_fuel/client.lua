...
RegisterNetEvent('lds_fuel:open')
AddEventHandler('lds_fuel:open',function(vehicle,data)
	open(vehicle,data)
end)
...
RegisterNetEvent('lds_fuel:close')
AddEventHandler('lds_fuel:close',function()
	close()
end)
...
RegisterNetEvent("lds_fuel:syncfuel")
AddEventHandler("lds_fuel:syncfuel",function(index,change,FuelDecor)
...
RegisterNetEvent('lds_fuel:jerrycan')
AddEventHandler('lds_fuel:jerrycan',function()
...
RegisterNetEvent('lds_fuel:refuelFromPump')
AddEventHandler('lds_fuel:refuelFromPump',function(pumpObject,ped,vehicle)
...
RegisterNetEvent('lds_fuel:fuelevent')
AddEventHandler('lds_fuel:fuelevent',function(pumpObject,ped,vehicle)
	...
	if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 3.5 then
		if isNearPump then
			...
			isFueling = true
			TriggerEvent('lds_fuel:refuelFromPump',isNearPump,ped,vehicle)
		end
	end
end)
...
function PopUI(name,v,reqdist,event,arg,server)
    ...
    TriggerEvent('lds_popui:showui',table)
		...
		while not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) do
			...
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 3.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle,-1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 99 and canFuel and isNearPump then
							DrawtextUI("Press [E] to Re Fuel Vehicle",stringCoords,3.5,'lds_fuel:open',{vehicle,output},false,false,'E')
							--PopUI("Re Fuel Vehicle",stringCoords,3.5,'lds_fuel:open',{vehicle,output},false)
						elseif canFuel and GetVehicleFuelLevel(vehicle) < 99 then
							DrawtextUI("Press [E] to Re Fuel Vehicle (PETROL CAN)",stringCoords,3.5,'lds_fuel:refuelFromPump',{false,ped,vehicle},false,false,'E')
				...
				elseif isNearPump then
					local stringCoords = GetEntityCoords(isNearPump)
					DrawtextUI("Press [E] to Buy Jerry Can",stringCoords,3.5,'lds_fuel:payfuel',{10000,true},true,false,'E')
					--PopUI("Buy Jerry Can",stringCoords,3.5,'lds_fuel:payfuel',{10000,true},true)
				end
			end
			Citizen.Wait(sleep)
		end
...
RegisterNetEvent('lds_fuel:Notify')
AddEventHandler('lds_fuel:Notify',function(msg)
...
RegisterNUICallback('pay', function(data, cb)
	...
	if not paid then
		if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(vehicle)) < 5 then
			TriggerServerEvent('lds_fuel:payfuel',math.floor(new_perc),false,VehToNet(vehicle),math.floor(new_perc),Config.FuelDecor,pumpLocation)
			paid = true
		end
	end
end)
