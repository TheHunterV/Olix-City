...
xsound = function()
	...
	if not XSOUND then
		play = exports.lds_mp3.PlayUrl
		xSound = exports.lds_mp3
	elseif XSOUND then
...
ChooseCharacter = function(slot)
	...
	local login = callback('lds_multicharacter:choosecharacter', slot)
	...
	elseif Config.skin == 'fivem-appearance' then
		...
		exports['fivem-appearance']:startPlayerCustomization(function (appearance)
			if (appearance) then
				...
				local save = callback('lds_multicharacter:saveappearance', appearance)
				finished = true
...
RegisterNUICallback('nuicb', function(data)
	...
	if data.msg == 'create' then
		...
		callback('lds_multicharacter:createcharacter', {info = data.info, slot = data.slot})
		Cleanups()
	...
	if data.msg == 'deletechar' then
		...
		callback('lds_multicharacter:deletecharacter', data.slot)
