...
RegisterCommand('testnotify', function() -- sample
    ...
    TriggerEvent('lds_notify:Notify','success','Hello There', 'Your Brain is now Functional', a)
    TriggerEvent('lds_notify:Notify','info','Loading..', 'We are now Downloading your New Brain', a)
    TriggerEvent('lds_notify:Notify','error','Oh no!', 'There is Problem with your brain', a)
    TriggerEvent('lds_notify:Notify','warning','Whats happening?', 'i think there is a problem with my Brain', a)
    TriggerEvent('lds_notify:Notify','default','lds Notify', 'This is a Free Notify Available at Github', a)
end, false)

RegisterNetEvent('lds_notify:Notify') -- this is all of it so simple :D
AddEventHandler('lds_notify:Notify', function(type,title,msg,options)
    local refresh = false
    local conf = config
    if options ~= nil then
