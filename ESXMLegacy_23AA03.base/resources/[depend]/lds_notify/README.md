...
```
    ...
    TriggerEvent('lds_notify:Notify','success','Hello There', 'Your Brain is now Functional', option)
    TriggerEvent('lds_notify:Notify','info','Loading..', 'We are now Downloading your New Brain', option)
    TriggerEvent('lds_notify:Notify','error','Oh no!', 'There is Problem with your brain', option)
    TriggerEvent('lds_notify:Notify','warning','Whats happening?', 'i think there is a problem with my Brain', option)
    TriggerEvent('lds_notify:Notify','default','lds Notify', 'This is a Free Notify Available at Github', option)
```
- Without Options: (will use config)
```
TriggerEvent('lds_notify:Notify','default','lds Notify', 'This is a Free Notify Available at Github')
```
# Preview
