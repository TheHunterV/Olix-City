...
RegisterNetEvent('lds_contextmenu:insert')
AddEventHandler('lds_contextmenu:insert', function(table,title,entity,clear,header)
    local table = {
...
RegisterNetEvent('lds_contextmenu:insertmulti')
AddEventHandler('lds_contextmenu:insertmulti', function(table,entity,clear,header)
...
RegisterNetEvent('lds_contextmenu:show')
AddEventHandler('lds_contextmenu:show', function(table,title,entity,clear)
    Wait(100)
...
RegisterNetEvent('lds_contextmenu:clear')
AddEventHandler('lds_contextmenu:clear', function()
...
function ReceiveData(data)
    ...
    if data.variables ~= nil and data.variables.onclickcloseui then -- close the ui before any trigger event, prevent UI bug from other NUI focus.
        ...
        if config.UsingRayzoneTarget then
            TriggerEvent('lds_rayzone:close')
        end
        open = false
    end
    ...
    elseif data.type == 'shop' then
        TriggerServerEvent("lds_rayzone:shop",data,data.inputval)
        SendNUIMessage({type = "reset", content = true})
        SetNuiFocus(false,false)
        SetNuiFocusKeepInput(false)
        if config.UsingRayzoneTarget then
    ...
    if data.variables ~= nil and data.variables.onclickcloseui then
        TriggerEvent('lds_contextmenu:close')
    end
end

RegisterNetEvent('lds_contextmenu:close')
AddEventHandler('lds_contextmenu:close', function()
    print("GAGO")
    close()
end)
