...
function makeToast(target, subject, message)
    TriggerClientEvent('esx_uteknark_coke:make_toast', target, subject, message)
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('esx_uteknark_coke:do', who, what, where)
end

RegisterNetEvent('esx_uteknark_coke:success_plant')
AddEventHandler ('esx_uteknark_coke:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('esx_uteknark_coke:log')
AddEventHandler ('esx_uteknark_coke:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('esx_uteknark_coke:test_forest')
AddEventHandler ('esx_uteknark_coke:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('esx_uteknark_coke:attempt_plant', source)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_coke:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_coke:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_coke:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_coke:pyromaniac', source)
        end
    end,
}
