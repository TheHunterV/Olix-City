...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `uteknark_opium` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('esx_uteknark_opium:planted',-1, id, location, stage)
    ...
    import = function(instance, id, location, stage, time, soil)
        ...
        if not success then
            Citizen.Trace(string.format("uteknark_opium failed to import plant with ID %i into octree\n", id))
        end
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark_opium` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('esx_uteknark_opium:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            ...
            { ['@id'] = id },
            function()
                TriggerClientEvent('esx_uteknark_opium:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('esx_uteknark_opium:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('esx_uteknark_opium:bulk_data', target, forest)
        else
            TriggerServerEvent('esx_uteknark_opium:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('esx_uteknark_opium:request_data')
    AddEventHandler ('esx_uteknark_opium:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('esx_uteknark_opium:remove')
    AddEventHandler ('esx_uteknark_opium:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('esx_uteknark_opium:remove', src, plantID)
        end
    end)

    RegisterNetEvent('esx_uteknark_opium:frob')
    AddEventHandler ('esx_uteknark_opium:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
            ...
            if distance <= Config.Distance.Interact then
                ...
                if stageData.interact then
                    if stageData.yield then
                        ...
                        elseif GiveItem(src, Config.Items.Product, yield) then
                            cropstate:remove(plantID)
                            doScenario(src, 'Frob', plantLocation)
                            if seeds > 0 and GiveItem(src, Config.Items.Seed, seeds) then
                                makeToast(src, _U('interact_text'), _U('interact_harvested', yield, seeds))
                        ...
                        if #Growth > plant.data.stage then
                            if not Config.Items.Tend or TakeItem(src, Config.Items.Tend) then
                                ...
                                doScenario(src, 'Frob', plantLocation)
                            elseif not Config.Items.Tend or TakeItem(src, Config.Items.Tend) then
                                makeToast(src, _U('interact_text'), _U('interact_tended'))
                                cropstate:update(plantID, plant.data.stage + 1)
                                doScenario(src, 'Frob', plantLocation)
                            else
    ...
    RegisterNetEvent('esx_uteknark_opium:bulk_data')
    AddEventHandler ('esx_uteknark_opium:bulk_data', function(forest)
    ...
    RegisterNetEvent('esx_uteknark_opium:planted')
    AddEventHandler ('esx_uteknark_opium:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('esx_uteknark_opium:update')
    AddEventHandler ('esx_uteknark_opium:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('esx_uteknark_opium:removePlant')
    AddEventHandler ('esx_uteknark_opium:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
