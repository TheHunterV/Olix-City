...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `uteknark_coke` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('esx_uteknark_coke:planted',-1, id, location, stage)
    ...
    import = function(instance, id, location, stage, time, soil)
        ...
        if not success then
            Citizen.Trace(string.format("uteknark_coke failed to import plant with ID %i into octree\n", id))
        end
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark_coke` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('esx_uteknark_coke:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            ...
            { ['@id'] = id },
            function()
                TriggerClientEvent('esx_uteknark_coke:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('esx_uteknark_coke:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('esx_uteknark_coke:bulk_data', target, forest)
        else
            TriggerServerEvent('esx_uteknark_coke:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('esx_uteknark_coke:request_data')
    AddEventHandler ('esx_uteknark_coke:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('esx_uteknark_coke:remove')
    AddEventHandler ('esx_uteknark_coke:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('esx_uteknark_coke:remove', src, plantID)
        end
    end)

    RegisterNetEvent('esx_uteknark_coke:frob')
    AddEventHandler ('esx_uteknark_coke:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('esx_uteknark_coke:bulk_data')
    AddEventHandler ('esx_uteknark_coke:bulk_data', function(forest)
    ...
    RegisterNetEvent('esx_uteknark_coke:planted')
    AddEventHandler ('esx_uteknark_coke:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('esx_uteknark_coke:update')
    AddEventHandler ('esx_uteknark_coke:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('esx_uteknark_coke:removePlant')
    AddEventHandler ('esx_uteknark_coke:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
