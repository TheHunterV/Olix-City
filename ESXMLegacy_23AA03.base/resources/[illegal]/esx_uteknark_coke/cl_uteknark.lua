...
function serverlog(...)
    TriggerServerEvent('esx_uteknark_coke:log',...)
end

RegisterNetEvent('esx_uteknark_coke:make_toast')
AddEventHandler ('esx_uteknark_coke:make_toast', function(subject,message)
...
RegisterNetEvent('esx_uteknark_coke:do')
AddEventHandler ('esx_uteknark_coke:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknark_coke:attempt_plant')
AddEventHandler ('esx_uteknark_coke:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknark_coke:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknark_coke:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknark_coke:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('esx_uteknark_coke:toggle_debug')
AddEventHandler ('esx_uteknark_coke:toggle_debug', function()
...
RegisterNetEvent('esx_uteknark_coke:pyromaniac')
AddEventHandler ('esx_uteknark_coke:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknark_coke:groundmat')
AddEventHandler ('esx_uteknark_coke:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('esx_uteknark_coke:test_forest')
AddEventHandler ('esx_uteknark_coke:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('esx_uteknark_coke:test_forest', forest)
end)
