...
function InitializeChopShop()
	...
	while true do
		...
		TriggerClientEvent('lds_chopshop:intializeChopShop', -1, scrapList)
...
AddEventHandler('esx:playerLoaded', function(playerId)
	TriggerClientEvent('lds_chopshop:intializeChopShop', playerId, carList)
end)

-- Callback to check if vehicle is owned:
ESX.RegisterServerCallback('lds_chopshop:isVehicleOwned',function(source, cb, plate)
...
ESX.RegisterServerCallback('lds_chopshop:getCopsCount',function(source, cb)
	local count = GetCopsCount()
	cb(count)
end)

-- Event to delete owned vehicle:
RegisterServerEvent('lds_chopshop:deleteOwnedVehicle')
AddEventHandler('lds_chopshop:deleteOwnedVehicle',function(plate)
    MySQL.Async.execute('DELETE FROM owned_vehicles WHERE plate = @plate', {['@plate'] = plate})
end)

-- Event for scrap vehicle payment:
RegisterServerEvent('lds_chopshop:getPayment')
AddEventHandler('lds_chopshop:getPayment',function(scrapCar, percent)
	...
	if cfg.cash.enable then
		...
		TriggerClientEvent('lds_chopshop:ShowNotifyESX',source, Lang['cash_reward']:format(money))
	end
	-- Item Reward:
	...
	if Config.ChopShop.Settings.cooldown.scrap.enable then 
		TriggerEvent('lds_chopshop:addCooldown', xPlayer.source, 'scrap')
	end
end)

-- Server Event for selecting risk grade:
RegisterServerEvent('lds_chopshop:selectRiskGrade')
AddEventHandler('lds_chopshop:selectRiskGrade', function(label, grade, job_fees, cops, vehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if money >= job_fees then
		...
		if police >= cops then
			...
			TriggerClientEvent('lds_chopshop:BrowseAvailableJobs', xPlayer.source, 0, grade, veh)
			TriggerClientEvent('lds_chopshop:ShowNotifyESX', xPlayer.source, ((Lang['paid_for_job']):format(job_fees, label)))
		else
			TriggerClientEvent('lds_chopshop:ShowNotifyESX', xPlayer.source, Lang['not_enough_police'])
		end
	else
		TriggerClientEvent('lds_chopshop:ShowNotifyESX', xPlayer.source, Lang['not_enough_money'])
	end
end)
...
RegisterServerEvent('lds_chopshop:syncDataSV')
AddEventHandler('lds_chopshop:syncDataSV',function(data)
    TriggerClientEvent('lds_chopshop:syncDataCL', -1, data)
end)

-- Server Event for Job Reward:
RegisterServerEvent('lds_chopshop:JobCompleteSV')
AddEventHandler('lds_chopshop:JobCompleteSV',function(payout, percent)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if Config.ChopShop.Settings.cooldown.thiefjob.enable then 
		TriggerEvent('lds_chopshop:addCooldown', xPlayer.source, 'thief')
	end
	TriggerClientEvent('lds_chopshop:ShowNotifyESX', xPlayer.source, ((Lang['reward_msg']):format(money)))
end)

-- Event to trigger police notifications:
RegisterServerEvent('lds_chopshop:PoliceNotifySV')
AddEventHandler('lds_chopshop:PoliceNotifySV', function(targetCoords, streetName)
	TriggerClientEvent('lds_chopshop:PoliceNotifyCL', -1, (Lang['police_notify']):format(streetName))
	TriggerClientEvent('lds_chopshop:PoliceNotifyBlip', -1, targetCoords)
end)
...
RegisterServerEvent('lds_chopshop:addCooldown')
AddEventHandler('lds_chopshop:addCooldown',function(source, type)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('lds_chopshop:hasCooldown', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	if not CheckCooldownTimer(xPlayer.identifier, type) then
		...
		TriggerClientEvent('lds_chopshop:ShowNotifyESX', xPlayer.source, msg:format(GetCooldownTimer(xPlayer.identifier), type))
		cb(true)
	end
end)
