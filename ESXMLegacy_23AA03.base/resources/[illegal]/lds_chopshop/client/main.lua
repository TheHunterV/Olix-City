...
RegisterNetEvent('lds_chopshop:intializeChopShop')
AddEventHandler('lds_chopshop:intializeChopShop', function(scrapList)
...
function ChopShopMainMenu()
	...
	function(data, menu)
		...
		if data.current.value == 'scraplist' then
			menu.close()
			ESX.TriggerServerCallback('lds_chopshop:getCopsCount', function(cops) 
				if cops >= Config.ChopShop.Police.minCops then
					ESX.TriggerServerCallback('lds_chopshop:hasCooldown', function(cooldown)
...
function CarThiefMainMenu()
	...
	function(data, menu)
		...
		TriggerServerEvent('lds_chopshop:selectRiskGrade', selected.name, selected.value, selected.job_fees, selected.cops, selected.vehicles)
		ESX.UI.Menu.CloseAll()
		interacting = false
	end, function(data, menu)
		menu.close()
...
RegisterNetEvent('lds_chopshop:BrowseAvailableJobs')
AddEventHandler('lds_chopshop:BrowseAvailableJobs',function(spot, grade, car)
...
function CarThiefJob(id, grade, car)
	...
	-- update config state:
	job.inUse = true
	TriggerServerEvent('lds_chopshop:syncDataSV', Config.ThiefJobs)
	-- create job blip:
	...
	while true do
		...
		if end_thiefJob then 
			if thiefjob_done then 
				TriggerServerEvent('lds_chopshop:JobCompleteSV', car.payout, veh_health)
...
RegisterNetEvent('lds_chopshop:syncDataCL')
AddEventHandler('lds_chopshop:syncDataCL',function(data)
...
function ScrapVehicle()
	...
	if job_veh ~= nil or veh_lockpicked then
		...
		if Config.ChopShop.Settings.ownedVehicles.delete then
			ESX.TriggerServerCallback('lds_chopshop:isVehicleOwned', function(owned)
				if owned then
					TriggerServerEvent('lds_chopshop:deleteOwnedVehicle', plate)
				end
			end, plate)
		end
		-- reward:
		local data = GetScrapVehicleDetails(GetEntityModel(scrap_vehicle))
		TriggerServerEvent('lds_chopshop:getPayment', data, round(GetEntityHealth(scrap_vehicle)/10, 0))
		if Config.ChopShop.Settings.usePhoneMSG then
