...
function makeToast(target, subject, message)
    TriggerClientEvent('esx_uteknark_opium:make_toast', target, subject, message)
end

function makeToast_Opium(target, subject, message)
    TriggerClientEvent('esx_uteknark_opium:make_toast_opium', target, subject, message)
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('esx_uteknark_opium:do', who, what, where)
end

RegisterNetEvent('esx_uteknark_opium:success_plant')
AddEventHandler ('esx_uteknark_opium:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('esx_uteknark_opium:success_plant_opium')
AddEventHandler ('esx_uteknark_opium:success_plant_opium', function(location, soil)
    local src = source
    if oneSyncEnabled and false then -- "and false" because something is weird in my OneSync stuff
        local ped = GetPlayerPed(src)
        --log('ped:',ped)
        ...
        if distance > Config.Distance.Interact then
            ...
            makeToast_Opium(src, _U('planting_text'), _U('planting_too_far'))
            return
        end
    end
    if soil and Config.Soil[soil] then
        ...
        if #hits == 0 then
			if TakeItem(src, Config.Items.Seed) then
                if plantSeed(location, soil) then
                    makeToast_Opium(src, _U('planting_text'), _U('planting_ok'))
                    doScenario(src, 'Plant', location)
                else
                    GiveItem(src, Config.Items.Seed)
                    makeToast_Opium(src, _U('planting_text'), _U('planting_failed'))
                end
            else
                makeToast_Opium(src, _U('planting_text'), _U('planting_no_seed'))
            end
        else
            makeToast_Opium(src, _U('planting_text'), _U('planting_too_close'))
        end
    else
        makeToast_Opium(src, _U('planting_text'), _U('planting_not_suitable_soil'))
    end
end)

RegisterNetEvent('esx_uteknark_opium:log')
AddEventHandler ('esx_uteknark_opium:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('esx_uteknark_opium:test_forest')
AddEventHandler ('esx_uteknark_opium:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('esx_uteknark_opium:attempt_plant', source)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_opium:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_opium:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_opium:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('esx_uteknark_opium:pyromaniac', source)
        end
    end,
}
