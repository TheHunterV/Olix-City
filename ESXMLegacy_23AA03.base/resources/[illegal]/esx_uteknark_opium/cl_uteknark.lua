...
function serverlog(...)
    TriggerServerEvent('esx_uteknark_opium:log',...)
end

RegisterNetEvent('esx_uteknark_opium:make_toast')
AddEventHandler ('esx_uteknark_opium:make_toast', function(subject,message)
...
RegisterNetEvent('esx_uteknark_opium:do')
AddEventHandler ('esx_uteknark_opium:do', function(scenarioName, location)
...
RegisterNetEvent('esx_uteknark_opium:attempt_plant')
AddEventHandler ('esx_uteknark_opium:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('esx_uteknark_opium:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('esx_uteknark_opium:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('esx_uteknark_opium:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('esx_uteknark_opium:toggle_debug')
AddEventHandler ('esx_uteknark_opium:toggle_debug', function()
...
RegisterNetEvent('esx_uteknark_opium:pyromaniac')
AddEventHandler ('esx_uteknark_opium:pyromaniac',function(location)
...
RegisterNetEvent('esx_uteknark_opium:groundmat')
AddEventHandler ('esx_uteknark_opium:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
AddEventHandler ('esx_uteknark_opium:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('esx_uteknark_opium:test_forest', forest)
end)
